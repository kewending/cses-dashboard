  ...
  return (
    <div className="w-full max-w-3xl mx-auto h-[calc(100vh-8rem)] flex flex-col animate-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center gap-4 mb-12 text-[var(--color-text-muted)] mt-8">
        <a href="/" className="hover:text-white transition-colors">← Back to Command Center</a>
        <span>/</span>
      ...
      <input 
        ...
        className="w-full bg-transparent text-4xl font-bold text-white outline-none placeholder:text-[var(--color-text-muted)] mb-8"
      ...
      <div className="flex justify-between items-center mt-6 text-sm text-[var(--color-text-muted)] border-t border-[var(--color-glass-border)] pt-4">
        <span>32 words</span>
        <button className="bg-[var(--color-accent)] text-white px-6 py-2 rounded-full font-medium hover:brightness-110 transition-all shadow-[0_0_15px_var(--color-accent-glow)] hover:shadow-[0_0_25px_var(--color-accent-glow)]">
          Save Entry
        </button>
      </div>
    </div>
